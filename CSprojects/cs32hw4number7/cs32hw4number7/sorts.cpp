...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
	...
	int unsorted = 1;
	int loc = 0;
	for (; unsorted < s.size(); unsorted++) {
		Student temp = s[unsorted];
		loc = unsorted;
