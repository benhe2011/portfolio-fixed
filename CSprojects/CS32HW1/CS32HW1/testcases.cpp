...
void testone(int n)
{
	...
	default: {
		...
		assert(!s.insert(1, DUMMY) && s.size() == 0);
	} break; case 46: {
		assert(!s.insert(-1, DUMMY) && s.size() == 0);
	} break; case 47: {
		s.insert(0, DUMMY);
		assert(!s.insert(2, DUMMY) && s.size() == 1);
	} break; case 48: {
		s.insert(0, DUMMY);
		...
		for (size_t k = 0; k < SPEC_MAX; k++)
			assert(s.insert(k, DUMMY));
	} break; case 77: {
		for (size_t k = 0; k < SPEC_MAX; k++)
			assert(s.insert(k, DUMMY));
		assert(!s.insert(SPEC_MAX, DUMMY));
	} break; case 78: {
		for (size_t k = 0; k < SPEC_MAX - 1; k++)
			assert(s.insert(k, ARRAY[0]));
		...
		for (size_t k = 0; k < CAPACITY; k++)
			assert(s2.insert(k, DUMMY));
		assert(!s2.insert(CAPACITY, DUMMY));
	} break; case 82: {
